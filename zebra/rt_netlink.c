	...
	int cmd)
{
  ...
  _netlink_route_build_singlepath(routedesc, bytelen, nhlfe->nexthop,  nlmsg, rtmsg, req_size, cmd);
}
        ...
        union g_addr **src)
{
  ...
  family = NHLFE_FAMILY (nhlfe);
  bytelen = (family == AF_INET ? 4 : 16);
  _netlink_route_build_multipath(routedesc, bytelen, nhlfe->nexthop, rta, rtnh, rtmsg, src);
}
...
netlink_mpls_multipath (int cmd, zebra_lsp_t *lsp)
{
  ...
  int nexthop_num;
  const char *routedesc;
 //FIXME struct zebra_ns *zns = zebra_ns_lookup (NS_DEFAULT);
  ...
  /* Talk to netlink socket. */
 //FIXME return netlink_talk (&req.n, &zns->netlink_cmd, zns);
   return netlink_talk (&req.n, &zvrf->netlink_cmd, zvrf);
